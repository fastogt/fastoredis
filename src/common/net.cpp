...
namespace common
{
    common::string16 convertToString16(const net::hostAndPort& host)
    {
        static const uint16_t size_buff = 512;
        char buff[size_buff] = {0};
        strings::SafeSNPrintf(buff, size_buff, "%s:%u", host.first.c_str(), host.second);
        return convertToString16(buff);
    }

    template<>
    net::hostAndPort convertFromString16(const common::string16& host)
    {
        net::hostAndPort res;
        size_t del = host.find_first_of(':');
        if(del != std::string::npos){
            res.first = host.substr(0, del);
            res.second = convertFromString16<uint16_t>(host.substr(del + 1));
        }

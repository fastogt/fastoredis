...
bool ReplaceChars(const string16& input,
                  const common::StringPiece16& replace_chars,
...
bool ReplaceChars(const std::string& input,
                  const common::StringPiece& replace_chars,
...
bool RemoveChars(const string16& input,
                 const common::StringPiece16& remove_chars,
                 string16* output) {
  return ReplaceChars(input, remove_chars.as_string(), string16(), output);
}

bool RemoveChars(const std::string& input,
                 const common::StringPiece& remove_chars,
                 std::string* output) {
...
bool TrimString(const string16& input,
                const common::StringPiece16& trim_chars,
                string16* output) {
...
bool TrimString(const std::string& input,
                const common::StringPiece& trim_chars,
                std::string* output) {
  return TrimStringT(input, trim_chars.as_string(), TRIM_ALL, output) !=
      TRIM_NONE;
}
...
TrimPositions TrimWhitespace(const string16& input,
                             TrimPositions positions,
                             string16* output) {
  return TrimStringT(input, common::string16(kWhitespaceUTF16), positions,
                     output);
}
...
bool EqualsASCII(const string16& a, const common::StringPiece& b) {
...
bool StartsWithASCII(const std::string& str,
  ...
  if (case_sensitive)
    ...
    return common::strncasecmp(str.c_str(), search.c_str(), search.length()) == 0;
}
...
bool StartsWithT(const STR& str, const STR& search, bool case_sensitive) {
  if (case_sensitive) {
    ...
    return std::equal(search.begin(), search.end(), str.begin(),
                      common::CaseInsensitiveCompare<typename STR::value_type>());
  }
}
...
string16 FormatBytesUnlocalized(int64_t bytes) {
  ...
  if (bytes != 0 && dimension > 0 && unit_amount < 100) {
    common::snprintf(buf, arraysize(buf), "%.1lf%s", unit_amount,
                   kByteStringsUnlocalized[dimension]);
  } else {
    common::snprintf(buf, arraysize(buf), "%.0lf%s", unit_amount,
                   kByteStringsUnlocalized[dimension]);
  }

  return common::ASCIIToUTF16(buf);
}
...
std::string ReplaceStringPlaceholders(const common::StringPiece& format_string,
...
bool MatchPattern(const common::StringPiece& eval,
...
size_t common::strlcpy(char* dst, const char* src, size_t dst_size) {
  return lcpyT<char>(dst, src, dst_size);
}
size_t common::wcslcpy(wchar_t* dst, const wchar_t* src, size_t dst_size) {

...
{
    DataBaseInfo::DataBaseInfo(const unicode_string &name, size_t size)
    ...
    ServerInfo::Server::Server(const unicode_string& server_text)
        : redis_version_(), redis_git_sha1_(), redis_git_dirty_(), redis_mode_(), os_(),
        arch_bits_(0), multiplexing_api_(), gcc_version_() ,process_id_(0),
        run_id_(), tcp_port_(0), uptime_in_seconds_(0), uptime_in_days_(0), lru_clock_(0)
    {
        const unicode_string &src = server_text;
        size_t pos = 0;
        size_t start = 0;
        while((pos = src.find(UTEXT("\r\n"), start)) != unicode_string::npos){
            unicode_string line = src.substr(start, pos-start);
            size_t delem = line.find_first_of(':');
            unicode_string field = line.substr(0, delem);
            unicode_string value = line.substr(delem + 1);
    ...
    ServerInfo::Clients::Clients(const unicode_string& client_text)
        : connected_clients_(0), client_longest_output_list_(0),
        client_biggest_input_buf_(0), blocked_clients_(0)
    {
        const unicode_string &src = client_text;
        size_t pos = 0;
        size_t start = 0;
        while((pos = src.find(UTEXT("\r\n"), start)) != unicode_string::npos){
            unicode_string line = src.substr(start, pos-start);
    ...
    ServerInfo::Memory::Memory(const unicode_string& memory_text)
        : used_memory_(0), used_memory_human_(), used_memory_rss_(0), used_memory_peak_(0),
          used_memory_peak_human_(), used_memory_lua_(0),mem_fragmentation_ratio_(0), mem_allocator_()
    {
        const unicode_string &src = memory_text;
        size_t pos = 0;
        size_t start = 0;
        while((pos = src.find(UTEXT("\r\n"), start)) != unicode_string::npos){
            unicode_string line = src.substr(start, pos-start);
    ...
    ServerInfo::Persistence::Persistence(const unicode_string& persistence_text)
        : loading_(0), rdb_changes_since_last_save_(0), rdb_bgsave_in_progress_(0),
          rdb_last_save_time_(0), rdb_last_bgsave_status_(),
          rdb_last_bgsave_time_sec_(0), rdb_current_bgsave_time_sec_(0),
          aof_enabled_(0), aof_rewrite_in_progress_(0), aof_rewrite_scheduled_(0),
          aof_last_rewrite_time_sec_(0), aof_current_rewrite_time_sec_(0), aof_last_bgrewrite_status_()
    {
        const unicode_string &src = persistence_text;
        size_t pos = 0;
        size_t start = 0;
        while((pos = src.find(UTEXT("\r\n"), start)) != unicode_string::npos){
            unicode_string line = src.substr(start, pos-start);
    ...
    ServerInfo::Stats::Stats(const unicode_string& stats_text)
        : total_connections_received_(0), total_commands_processed_(0),
          instantaneous_ops_per_sec_(0), rejected_connections_(0),
          expired_keys_(0), evicted_keys_(0), keyspace_hits_(0),
          keyspace_misses_(0), pubsub_channels_(0),
          pubsub_patterns_(0), latest_fork_usec_(0)
    {
        const unicode_string &src = stats_text;
        size_t pos = 0;
        size_t start = 0;
        while((pos = src.find(UTEXT("\r\n"), start)) != unicode_string::npos){
            unicode_string line = src.substr(start, pos-start);
    ...
    ServerInfo::Replication::Replication(const unicode_string& replication_text)
        : role_(), connected_slaves_(0)
    {
        const unicode_string &src = replication_text;
        size_t pos = 0;
        size_t start = 0;
    ...
    ServerInfo::Cpu::Cpu(const unicode_string& cpu_text)
        : used_cpu_sys_(0), used_cpu_user_(0), used_cpu_sys_children_(0), used_cpu_user_children_(0)
    {
        const unicode_string &src = cpu_text;
        size_t pos = 0;
        size_t start = 0;
        while((pos = src.find(UTEXT("\r\n"), start)) != unicode_string::npos){
            unicode_string line = src.substr(start, pos-start);
    ...
    ServerInfo makeServerInfo(const unicode_string &content)
    {
        ...
        for(int i = 0; i < content.size(); ++i)
        {
            ...
            if(word == redisHeaders[j]){
                ...
                {
                    unicode_string part = content.substr(i + 1, pos - i - 1 );
                    switch(j)
                    {
                    case 0:
    ...
    {
        const unicode_string content = common::convert2string(root);
